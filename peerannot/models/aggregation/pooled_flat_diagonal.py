...
class VectorizedPooledFlatDiagonal(PooledDiagonalMultinomial):
    ...
    def _e_step(self) -> None:
        # Compute worker_labels_sum: sum over workers for each task and class
        worker_labels_sum = self.crowd_matrix.sum(
            axis=1,
        )  # shape (n_task, n_classes)

        # Compute diag_contrib_matrix: self.pi[j] ** worker_labels_sum[i, j] for all i, j
        diag_contrib_matrix = (
        ...
        # Compute powered terms: (n_task, n_classes, n_classes)
        powered_terms = off_diag_terms_expanded**worker_labels_sum_expanded

        # Take product over k (axis=2) to get (n_task, n_classes)
